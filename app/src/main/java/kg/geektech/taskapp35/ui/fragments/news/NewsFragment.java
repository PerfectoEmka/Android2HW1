package kg.geektech.taskapp35.ui.fragments.news;
...
public class NewsFragment extends Fragment {
    ...
    private void saveData() {
        ...
        news.setEmail(user.getEmail().toString());
    ...
    private void saveToFireStore(News news) {
        ...
        db.collection("news").add(news).addOnCompleteListener(task -> {
            if (task.isSuccessful()){
                Toast.makeText(requireContext(), "Success " + db.collection("news").getId(), Toast.LENGTH_SHORT).show();
            } else {
